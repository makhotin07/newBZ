...
class WorkspacesApi {
  ...
  async getCurrentMember(workspaceId: string): Promise<WorkspaceMember> {
    const response = await api.get(`/workspaces/${workspaceId}/members/me/`);
    return response.data;
  }
  ...
  async updateWorkspaceSettings(workspaceId: string, settings: Partial<WorkspaceSettings>): Promise<WorkspaceSettings> {
    const response = await api.patch(`/workspaces/${workspaceId}/workspace_settings/`, settings);
    return response.data;
  }
