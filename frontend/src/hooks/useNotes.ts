...
import { notesApi, Page, Tag, CreatePageData, UpdatePageData, Comment } from '../services/notesApi';
...
export const useUpdateComment = () => {
  ...
  return useMutation({
    mutationFn: ({ id, content }: { id: string; content: string }) =>
      notesApi.updateComment(id, content),
...
export const useDeleteComment = (pageId?: string) => {
  ...
  return useMutation({
    mutationFn: (id: string) => notesApi.deleteComment(id),
    onSuccess: () => {
      if (pageId) {
        queryClient.invalidateQueries({ queryKey: notesKeys.pageComments(pageId) });
      } else {
...
export const useResolveComment = (pageId?: string) => {
  ...
  return useMutation({
    mutationFn: ({ id, resolved }: { id: string; resolved: boolean }) => notesApi.resolveComment(id, resolved),
    onSuccess: () => {
      if (pageId) {
        queryClient.invalidateQueries({ queryKey: notesKeys.pageComments(pageId) });
      } else {
