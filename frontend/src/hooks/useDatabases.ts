...
import { databasesApi, Database, DatabaseRecord, DatabaseProperty, DatabaseView, CreateDatabaseData, UpdateDatabaseData, CreateDatabasePropertyData, CreateDatabaseRecordData, UpdateDatabaseRecordData } from '../services/databasesApi';
...
export const useUpdateDatabaseProperty = (databaseId: string) => {
  ...
  return useMutation({
    mutationFn: ({ propertyId, data }: { propertyId: string; data: Partial<CreateDatabasePropertyData> }) =>
      databasesApi.updateDatabaseProperty(propertyId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: databaseKeys.properties(databaseId) });
...
export const useDeleteDatabaseProperty = (databaseId: string) => {
  ...
  return useMutation({
    mutationFn: (propertyId: string) => databasesApi.deleteDatabaseProperty(propertyId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: databaseKeys.properties(databaseId) });
...
export const useDatabaseRecords = (databaseId: string, params?: {
  ...
  return useQuery({
    ...
    queryFn: () => databasesApi.getDatabaseRecords(databaseId, params),
    enabled: !!databaseId,
    placeholderData: keepPreviousData,
  });
};

export const useDatabaseRecord = (recordId: string) => {
  return useQuery({
    queryKey: databaseKeys.record(recordId),
    queryFn: () => databasesApi.getDatabaseRecord(recordId),
    enabled: !!recordId,
  });
};
...
export const useUpdateDatabaseRecord = (databaseId: string) => {
  ...
  return useMutation({
    mutationFn: ({ recordId, data }: { recordId: string; data: UpdateDatabaseRecordData }) =>
      databasesApi.updateDatabaseRecord(recordId, data),
...
export const useDeleteDatabaseRecord = (databaseId: string) => {
  ...
  return useMutation({
    mutationFn: (recordId: string) => databasesApi.deleteDatabaseRecord(recordId),
    onSuccess: (_, deletedId) => {
      // Remove from cache
